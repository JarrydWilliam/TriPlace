...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/communities/category/:category", async (req, res) => {
    try {
      ...
      const communities = await storage.getCommunitiesByCategory(category);
      res.json(communities);
    } catch (error) {
  ...
  app.delete("/api/communities/:communityId/leave", async (req, res) => {
    try {
      const communityId = parseInt(req.params.communityId);
      const { userId } = req.body;
      ...
      if (success) {
        ...
        res.status(400).json({ error: "Failed to leave community" });
      }
    } catch (error) {
  ...
  app.get("/api/users/:userId/kudos/given", async (req, res) => {
    try {
      const userId = parseInt(req.params.userId);
      const kudos = await storage.getUserKudosGiven(userId);
      res.json(kudos);
    } catch (error) {
  ...
  app.post("/api/users/:userId/activity", async (req, res) => {
    try {
      ...
      const activity = await storage.addActivityItem(userId, type, content);
